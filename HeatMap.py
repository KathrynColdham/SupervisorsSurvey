...
def reader(filename):
	...
	#Creating dictionaries for the contour plot	
	x_dict = dict(list(enumerate(x)))
	y_dict = dict(list(enumerate(y)))

	x_dict_2 = {}
	y_dict_2 = {}

	#Obtaining two arrays, one for keys and one for values from the dictionary for x
	...
	for value in x_dict.values():
		...
		if exists:
			...
			if index_values != 0:
				...
				x_keys.append(index_values)

		index_values += 1			

		x_values.append(value)

	#Obtaining two arrays, one for keys and one for values from the dictionary for y
		
	y_values = []
	y_keys = []
	y_values_combinations = []

	index_values_3 = 0

	for value in y_dict.values():
		exists = value in y_values
		if exists:
			item = y_values_combinations.index(value)
			y_keys.append(item)
		else:
			y_keys.append(index_values_3)
			index_values_3 += 1
			y_values_combinations.append(value)
	...
	# Save the plot
	if not os.path.exists('Results_HeatMaps'):
		os.mkdir('Results_HeatMaps')

	pdf_name = 'HeatMap_' + PdfTitleStart + '_' + PdfTitleEnd + '.pdf'
		
	os.chdir('Results_HeatMaps')
